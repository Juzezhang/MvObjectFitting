...
class ObjectPoseFittingTest(nn.Module):
    ...
    def apply_transformation_1(self, deta):
        rots = rot6d_to_matrix(self.object_R_IMU + 1e-2 * deta)
    ...
    def forward(self, deta):

        if self.idx == self.ref:
            meshes_world = self.apply_transformation()
            loss_transl = 0
        else:
            meshes_world = self.apply_transformation_1(deta)
            loss_transl = torch.sum((self.object_init_T - self.translations.data) **2)*8e8
        ...
        T = self.Ts
        K = self.Ks
        image = (1 - self.image_refs_background) * \
                self.renderers(meshes_world=meshes_world.clone())[..., 3]
        loss += torch.sum((image - self.image_refs) ** 2, dim=(1, 2)) * self.object_masks_valid
        loss_offscreen += self.compute_offscreen_loss(meshes_world.clone().verts_padded(), R.float(), T.float()
                                                      , K.float()) * self.object_masks_valid

        loss = torch.mean((1. * loss + 10 * loss_offscreen) / self.object_masks_valid.sum()) + loss_transl
        images.append(image)
